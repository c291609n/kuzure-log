...
export default function App() {
  ...
  // Build day-aligned analysis records: each row is a 崩れ day, paired with THAT
  // morning's sleep/fatigue (from the previous record), not the same-entry values.
  const analysisRecords = (list) => {
    const byDate = {};
    logs.forEach((l) => { byDate[dayKey(l.date)] = l; });
    return list.map((l) => {
      const prev = byDate[dayKey(prevDay(l.date))];
      return {
        日付: prevDay(l.date),
        その朝の睡眠: prev ? sleepLabel(prev.sleep) : "記録なし",
        その朝の疲労: prev ? fatigueLabel(prev.fatigue) : "記録なし",
        イベント: (l.events || []).join("・"),
        イベントメモ: l.eventMemo || "",
        崩れ: l.kuzure ? "あり" : "なし",
        崩れの程度: l.kuzure ? kuzureLabelOf(l.kuzureLevel) : "",
        崩れ行動: (l.actions || []).join("・"),
        気持ち: (l.motives || []).join("・"),
...
各行は「崩れた日」1日分です。「その朝の睡眠」「その朝の疲労」はその崩れた日の朝の状態（＝崩れの前）なので、睡眠・疲労が崩れの原因かどうかを見るのに使えます。
...
${JSON.stringify(summary, null, 2)}`
  ...
  const runTypeAnalysis = async () => {
    ...
    setTypeLoading(true);
    try {
      const summary = logs.slice(0, 14).map((l, i, arr) => {
        const prev = arr.find((x) => dayKey(x.date) === dayKey(prevDay(l.date)));
        return {
          崩れ: l.kuzure ? "あり" : "なし",
          崩れの程度: l.kuzure ? kuzureLabelOf(l.kuzureLevel) : "",
          その朝の疲労: prev ? fatigueLabel(prev.fatigue) : "記録なし",
...
ログ：${JSON.stringify(summary)}`
          ...
          return (
            <div key={axis}>
              ...
              <div style={{ position: "relative", height: 8, borderRadius: 99, background: "#ece8e0" }}>
                <div style={{ position: "absolute", ...(leftWins ? { left: 0 } : { right: 0 }), top: 0, height: "100%", width: `${leftWins ? score : 100 - score}%`, background: color, borderRadius: 99, opacity: mid ? 0.4 : 0.85 }} />
  ...
  const runMonthlyReport = async () => {
    ...
    try {
      const data = analysisRecords(monthLogs);
      const res = await callAnalyze({
        model: "claude-sonnet-4-6",
        max_tokens: 1000,
        messages: [{
...
${JSON.stringify(data, null, 2)}`
  ...
  // Weekly trend: average 崩れ severity score (0-100) per week as a simple bar chart (lower = better).
  const renderTrend = () => {
    ...
    for (let w = count - 1; w >= 0; w--) {
      ...
      weeks.push({ start: ws, end: we, levelSum: 0, total: 0 });
    }
    logs.forEach((l) => {
      const [y, m, d] = dayKey(l.date).split("/").map(Number);
      const dt = new Date(y, m - 1, d);
      const wk = weeks.find((w) => dt >= w.start && dt <= w.end);
      if (wk) { wk.total++; wk.levelSum += (l.kuzureLevel != null ? l.kuzureLevel : (l.kuzure ? 50 : 0)); }
    });
          ...
          {weeks.map((wk, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", height: "100%" }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: wk.total === 0 ? "#ddd" : "#e0503a", marginBottom: 3 }}>{wk.total === 0 ? "–" : wk.score}</span>
                <div style={{ width: "72%", height: h, minHeight: wk.total === 0 ? 0 : 3, background: wk.score > 0 ? "#e0503a" : "#d9e8d9", borderRadius: "4px 4px 0 0" }} />
